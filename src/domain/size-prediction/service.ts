...
export async function getEstimation(
  ...
  try {
    ...
    return enhanceEstimation(result, inputFormat, outputFormat);
  } catch {
    return getFallbackEstimation(inputFormat, outputFormat, lossyMode);
  }
}

function enhanceEstimation(
...
function getFallbackEstimation(
  inputFormat: string,
  outputFormat: string,
  lossyMode: boolean
): EnhancedCompressionEstimationType {
